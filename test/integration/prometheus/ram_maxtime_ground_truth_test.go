	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			for namespace, allocationResponseItem := range apiResponse.Data[0] {
				...
				promNamespaceRAMByteMaxUsage := 0.0
				...
				promInput.Function = "max_over_time"
				...
				for _, promResponseItem := range promResponse.Data.Result {
					...
					if err != nil {
						...
						promNamespaceRAMByteMaxUsage += floatVal
					}
				}

			
				if promNamespaceRAMByteMaxUsage != allocationResponseItem.RawAllocationsOnly.RAMByteUsageMax {
					t.Errorf("RAM Byte Hours Sum does not match for prometheus %f and /allocation %f for namespace %s", promNamespaceRAMByteMaxUsage, allocationResponseItem.RawAllocationsOnly.RAMByteUsageMax, namespace)	
				} else {
